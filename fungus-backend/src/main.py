...
class MusicRecommendationFungus:
    ...
    def start(self):
        ...
        logging.info({"node_id": f"fungus-node-{FUNGUS_ID}", "event": "message_received", "details": {"from": FUNGUS_ID, "model": self.learning_group_id}, "timestamp": datetime.today().strftime('%Y-%m-%dT%H:%M:%S')})
        i = 0
        while True:
